...
delta_FD, plus file paths (not base64) for the vessel mask and skeleton images.
...
def predict(image_path, output_dir):
    ...
    cv2.imwrite(mask_path, (mask_2d * 255).astype(np.uint8))
    cv2.imwrite(skeleton_path, (skeleton_total.astype(np.uint8) * 255))
