...
public class AdherenceServiceTests
{
    ...
    public void GenerateSchedule_Creates_All_Doses_In_Clinic_Time()
    {
        ...
        logs.Select(log => log.ScheduledLocalDate).Should().BeEquivalentTo(
            new DateOnly(2026, 8, 17), new DateOnly(2026, 8, 17),
            new DateOnly(2026, 8, 18), new DateOnly(2026, 8, 18));
        logs.Select(log => log.ScheduledAt.Hour).Should().BeEquivalentTo(1, 13, 1, 13);
    }
